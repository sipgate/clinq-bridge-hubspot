...
const { fetchContacts, createClient } = require("./clinq-adapter-hubspot");

const CACHE_LIFETIME_MS = 5 * 1000;
const cache = [];
...
const populateCache = async (apiKey, client) => {
	...
	if (!cacheEntry.loaded) {
		console.log(
			`Contacts already loading for ${anonKey} (currently ${cacheEntry.list.length} contacts)`
		);
		return;
	}
